...
def get_area_info():
    ...
    # 将数据保存到redis中
    try:
        redis_store.setex("area_info", constants.AREA_INFO_REDIS_CAHAE_EXPIRES, resp_json)
    except Exception as e:
        current_app.logger.error(e)
...
def get_user_houses():
    ...
    try:
        # houses = House.filter_by(user_id=user_id)
...
def get_hous_index():
    ...
    try:
        ...
        current_app.logger.error(e)
        ret = None  # 异常时，ret在下面使用时会报错未定义，这里就给它定义一下
...
def get_house_detail(house_id):
    ...
    # 先从redis中获取
    try:
        ret = redis_store.get("huose_info_s%" % house_id)
    except Exception as e:
        current_app.logger.error(e)
        ret = None
    if ret:
...
# GET /api/v1.0/houses?sd=20191001&ed=20191007&aid=1&sk=new&p=1
